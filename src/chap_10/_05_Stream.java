...
public class _05_Stream {
    public static void main(String[] args) {
        ...
        // langList.add("python");
        // langList.add("java");
        langList = Arrays.asList("python", "java", "javascript", "c", "c++", "c#"); // 위 처럼 하나씩 추가해 줄 필요가 없음
        ...
        // 스트림 사용
        // 1. 중간 연산 : filter, map, sorted, distinct, skip, ...
        // 2. 최종 연산 : count, mun, max, sum, forEach, ...

        // 90 점 이상인 점수만 출력
        Arrays.stream(scores).filter(x -> x >= 90).forEach(x -> System.out.println(x));
        System.out.println("----------------------------------------");
        // Arrays.stream(scores).filter(x -> x >= 90).forEach(System.out::println);  // 위 문장과 동일하게 동작
        ...
        // 4글자 이하의 언어들은 모두 c라는 글자를 포함하는지 여부 : allMatch
        ...
        List<String> langListStartWithC = langList.stream().filter(x -> x.contains("c")).map(String::toUpperCase)
